...
do(State) ->
    ...
    RpcConfig = rebar_opts:get(Config, gpb_rpc, []),
    ...
    GpbOpts = proplists:get_value(gpb_opts, RpcConfig, []),
    
    [begin
         GpbModule = preload_pb(Filename, GpbOpts),
         try
             gen_rpc_router_mod(GpbModule, Options, RpcConfig, State)
         catch E:R ->
             rebar_utils:abort("Failed to generate Router ~p ~p: ~p ~nPlease run rebar3 protobuf compile first!~n",
                 [GpbModule, E, R])
         end
...
gen_rpc_router_mod(GpbModule, Options, RpcConfig, State) ->
    OutDir = proplists:get_value(out_dir, RpcConfig, "src"),
    ...
    [rebar_templater:new("gpb_rpc", Service, Force, State) || Service <- Services].
